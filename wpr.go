...
func (common *CommonConfig) Flags() []cli.Flag {
	return append(common.certConfig.Flags(),
		...
		cli.StringFlag{
			Name:        "logFile",
			Value:       "",
			Usage:       "Log File that the log output gets redirected to when running WPR",
			Destination: &common.logFile,
		},
	)
}
...
func (r *ReplayCommand) Flags() []cli.Flag {
	return append(r.common.Flags(),
		...
		cli.BoolFlag{
			Name: "disable_URL_fuzzy_matching",
			Usage: "Disables the partial matching logic based on querystrings" +
				"when no exact url matches are found",
			Destination: &r.disableURLFuzzyMatching,
		})
}
...
func (r *RecordCommand) Run(c *cli.Context) {
	...
	if len(r.common.logFile) != 0 {
		f, err := os.OpenFile(r.common.logFile, os.O_RDWR|os.O_CREATE|os.O_APPEND, 0666)
		if err != nil {
	...
	if err != nil {
		...
		log.Printf("Error creating TLSConfig: %v", err)
		os.Exit(1)
	}
...
func (r *ReplayCommand) Run(c *cli.Context) {
	...
	if err != nil {
		...
		log.Printf("Error opening archive file: %v", err)
		os.Exit(1)
	}
	log.Printf("Opened archive %s", archiveFileName)
	...
	if r.rulesFile != "" {
		...
		if err != nil {
			...
			log.Printf("Error opening rules file %s: %v\n", r.rulesFile, err)
			os.Exit(1)
		}
	...
	httpHandler := webpagereplay.NewReplayingProxy(archive, "http", r.common.transformers, r.quietMode, r.disableURLFuzzyMatching)
	httpsHandler := webpagereplay.NewReplayingProxy(archive, "https", r.common.transformers, r.quietMode, r.disableURLFuzzyMatching)
