...
func NewReplayingProxy(a *Archive, scheme string, transformers []ResponseTransformer, quietMode bool, disableURLFuzzyMatching bool) http.Handler {
	return &replayingProxy{a, scheme, transformers, quietMode, disableURLFuzzyMatching}
}
...
func (proxy *replayingProxy) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	_, storedResp, err := proxy.a.FindRequest(req, proxy.disableURLFuzzyMatching)
	if err != nil {
	...
	if _err == nil {
		...
		w.Header().Add("server-timing", " CurrentTime; dur="+strconv.FormatInt(time.Now().Unix(), 10))
		w.Header().Add("server-timing", " wpr; desc="+req.RemoteAddr+"->"+hostName+";dur="+strconv.FormatInt(elapsed, 10))
	}
	...
	logf("serving %s with %v response in %d time", req.URL, storedResp.StatusCode, time.Since(start).Milliseconds())
}
