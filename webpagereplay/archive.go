...
func (a *Archive) FindRequest(req *http.Request, disableURLFuzzyMatching bool) (*http.Request, *http.Response, error) {
	...
	if disableURLFuzzyMatching == false {
		...
		if bestURL != "" {
			fmt.Errorf("Fuzzy match : Request - %v, Response %v", req.URL, bestURL)
...
func (a *Archive) Merge(other *Archive) error {
	...
	err := other.ForEach(func(req *http.Request, resp *http.Response) error {
		foundReq, _, notFoundErr := a.FindRequest(req, false)
...
func (a *Archive) Add(method string, urlString string, mode AddMode) error {
	...
	if mode == AddModeAppend || mode == AddModeSkipExisting {
		if foundReq, _, notFoundErr := a.FindRequest(req, false); notFoundErr != ErrNotFound {
